...
filter_day['weekday'] = filter_day['weekday'].replace({0: 'Minggu', 1: 'Senin', 2: 'Selasa', 3: 'Rabu', 4: 'Kamis', 5: 'Jumat', 6: 'Sabtu'})
day_order = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu']
filter_day['weekday'] = pd.Categorical(filter_day['weekday'], categories=day_order, ordered=True)
...
st.text('Perbedaan Rata - Rata Total Penyewaan Sepeda pada Hari Kerja dan Libur')
...
weather = filter_day.pivot_table(index='weathersit', values='cnt', aggfunc='mean').reset_index()
weather['weathersit'] = weather['weathersit'].replace({1: 'Cerah', 2: 'Berawan', 3: 'Hujan Ringan'})
plt.figure(figsize=(10, 5))
sns.barplot(y='weathersit',
        x='cnt',
        data=weather,
        orient='h',
        color='blue')
...
users = filter_day.pivot_table(index='cnt', values=['casual', 'registered']).reset_index()
